...
export default function CategoryHistoryPanel({
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto mt-3 overflow-hidden rounded-3xl border border-black/10 bg-white shadow-[0_18px_60px_rgba(0,0,0,0.08)]">
      {/* subtle glow */}
      <div
              ...
              <ul className="divide-y divide-black/5">
                {categories.map((c) => (
                  <li key={c.id}>
                    <button
                      onClick={() => onSelect(c)}
                      className="group w-full text-left px-4 py-4 flex items-center justify-between gap-4 hover:bg-black/[0.02] transition"
                    >
                      ...
                      <span className="text-sm text-black/40 group-hover:translate-x-1 transition">
                        →
                      </span>
                    </button>
                  </li>
